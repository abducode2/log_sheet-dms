...
const TABLE = 'supplier_prequalifications'

const SPQ_PREFIX: Record<string,string> = {
  ARC:'J500-RWF-SPQ-ARC-',
  CIV:'J500-RWF-SPQ-CIV-',
  SUR:'J500-RWF-SPQ-SUR-',
  MEC:'J500-RWF-SPQ-MEC-',
  ELE:'J500-RWF-SPQ-ELE-',
  GEN:'J500-RWF-SPQ-GEN-',
}
...
const FIELDS  = [
  ...
  { key:'request_no',     label:'رقم الطلب',      type:'text',   required:true,
    prefixDynamic:{ fromField:'element', map:SPQ_PREFIX } },
...
function groupRows(rows: Row[]): Group[] {
  ...
  return Object.entries(map).map(([, group]) => {
    ...
    return {
      ...
      rows: sorted,
      no:          root.no,
      description: root.supplier_name ?? "",
...
export default function SupplierPage() {
  ...
  const [colFilters, setColFilters] = useState<Record<string,string>>({
    request_no:'', supplier_name:'', trade:'', element:'', submission_date:'', status:'', rev:''
  })
  ...
  const COL_HEADERS = [
    ...
    { key:'description', },   // removed description col
  ...
  return (
    <>
      <Topbar
        title="تأهيل الموردين — Supplier Pre-Qualification"
        sub={`HARAJ-IQC-ALRAWAF · إجمالي ${counts.ALL ?? 0} مورد`}
        actions={<>
          {isEditor && (
        ...
        <div className="toolbar" onClick={() => setOpenCol(null)}>
          ...
          {Object.values(colFilters).some(v => v) && (
            <button className="btn btn-ghost btn-sm" onClick={() =>
              setColFilters({ request_no:'', supplier_name:'', trade:'', element:'', submission_date:'', status:'', rev:'' })
            }>✕ مسح الفلاتر</button>
          )}
        </div>
...
{/* Excel-style filter headers */}
                ...
                {loading ? (
                  ...
                  <tr><td colSpan={11}>
                    <div className="empty-state">
                      ...
                      <div className="empty-title">لا يوجد موردون</div>
                      <div className="empty-sub">ابدأ بإضافة مورد جديد أو استورد من Excel</div>
                    </div>
                  </td></tr>
                ) : groups.map((group, gi) => (
                            ...
                            <label style={{
                              ...
                              <input type="file" accept="application/pdf" style={{display:'none'}}
                                onChange={async e => {
                                  ...
                                  const { url, error } = await uploadToCloudinary(file, 'p179/supplier-prequalifications')
      ...
      {showAdd && (
        <AddRecordModal table={TABLE} title="إضافة مورد جديد"
          fields={FIELDS} onClose={() => setShowAdd(false)}
          onSaved={() => { fetchData(); fetchCounts() }}
          autoNumber={{ field:'no', getNext: getNextNo }}
          onSaveAndGenerate={(record: Record<string, unknown>) => generateForm({ docType:'SPQ', titleAr:'تأهيل الموردين', fields:FIELDS, record })}/>
      )}
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ width:420 }}>
            ...
            <div style={{ background:'var(--bg3)', border:'1px solid #da363333', borderRadius:'var(--radius)', padding:16, marginBottom:20 }}>
              ...
              <div style={{ fontSize:13, marginBottom:8 }}>{confirmDel.supplier_name}</div>
