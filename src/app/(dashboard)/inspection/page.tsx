...
const IR_PREFIX: Record<string, string> = {
  ARC: 'J500-RWF-IR-ARC-',
  CIV: 'J500-RWF-IR-CIV-',
  SUR: 'J500-RWF-IR-SUR-',
  MEC: 'J500-RWF-IR-MEC-',
...
export default function InspectionPage() {
  ...
  async function getNextNo(): Promise<number> {
    const { data } = await supabase
      .from('inspection_requests').select('no')
  ...
  async function doSave(id: string, createRevision: boolean) {
    ...
    if (createRevision && editSt === 'C') {
      ...
      await supabase.from('inspection_requests').insert({
        id:              newId,
        no:              nextNo,
        ir_no:           row.ir_no ?? row.request_no,
        request_no:      row.ir_no ?? row.request_no,
    ...
    <>
                            ...
                            <div style={{display:'flex',flexDirection:'column',gap:3,alignItems:'center'}}>
                              <button
                                onClick={() => setViewingPdf({
                                  ...
                                  name: row.ir_no ?? row.request_no ?? 'document',
      ...
      {showAdd && (
        <AddRecordModal
          ...
          onSaveAndGenerate={record => generateForm({ docType:'IR', titleAr:'طلب فحص', fields:FIELDS, record })}
        />
      )}
