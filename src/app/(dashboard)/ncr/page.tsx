...
const FIELDS: FieldDef[] = [
  { key:'ncr_no',           label:'رقم NCR',           type:'text',   required:true },
  { key:'description',      label:'وصف عدم المطابقة',  type:'text',   required:true },
  ...
  { key:'corrective_action',label:'الإجراء التصحيحي',   type:'textarea' },
...
export default function NcrPage() {
  ...
  const [colFilters, setColFilters] = useState<Record<string,string>>({
    ncr_no: '', description: '', location: '', element: '', issue_date: '', ac_co: '', rev: ''
  })
  ...
  const fetchData = useCallback(async () => {
    ...
    if (s) {
      ...
      const matchingIds = new Set(
        rows
          .filter(r =>
            (r.ncr_no              ?? '').toLowerCase().includes(s) ||
            (r.description         ?? '').toLowerCase().includes(s) ||
            (r.location            ?? '').toLowerCase().includes(s) ||
            (r.element             ?? '').toLowerCase().includes(s) ||
            (r.ac_co               ?? '').toLowerCase().includes(s) ||
            (r.corrective_action   ?? '').toLowerCase().includes(s) ||
            (r.issue_date          ?? '').toLowerCase().includes(s) ||
  ...
  async function getNextNo(): Promise<number> {
    const { data } = await supabase
      .from('non_conformance_reports').select('no')
  ...
  function onEditDateChange(d: string) {
    ...
    const vt = calcVtime(row?.issue_date ?? null, d)
  ...
  async function doSave(id: string, createRevision: boolean) {
    ...
    if (createRevision && editSt === 'C') {
      ...
      await supabase.from('non_conformance_reports').insert({
        id:              newId,
        no:              nextNo,
        ncr_no:          row.ncr_no,
  ...
  async function exportExcel() {
    let q = supabase.from('non_conformance_reports').select('*').order('no')
    if (activeEl !== 'ALL') q = q.eq('element', activeEl)
  ...
  return (
    <>
      <Topbar
        ...
        sub={`MURCIA-2 Zone 06 · إجمالي ${counts.ALL ?? 0} تقرير`}
        actions={<>
      ...
      <div className="page-content">
        ...
        <div className="toolbar" onClick={() => setOpenCol(null)}>
          ...
          {Object.values(colFilters).some(v => v) && (
            <button className="btn btn-ghost btn-sm" onClick={() =>
              setColFilters({ ncr_no:'', description:'', location:'', element:'', issue_date:'', ac_co:'', rev:'' })
            }>
              ✕ مسح الفلاتر
            </button>
          )}
        ...
        <div className="table-wrap">
          <div className="table-header">
            ...
            <span className="table-meta">{total} تقرير</span>
          </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  {[
                    { key:'ncr_no',      label:'رقم NCR',           w:undefined },
                    ...
                    { key:'issue_date',  label:'تاريخ الإصدار',      w:undefined },
                ...
                {loading ? (
                  ...
                  <tr><td colSpan={12}>
                    <div className="empty-state">
                      ...
                      <div className="empty-sub">ابدأ بإضافة تقرير NCR جديد أو استورد من Excel</div>
                    </div>
                  </td></tr>
                ) : groups.map((group, gi) => (
                  group.rows.map((row, ri) => {
                    ...
                    return (
                      <tr key={row.id}
                        ...
                        {/* Issue date */}
                        <td className="cell-mono cell-muted">{row.issue_date ?? '—'}</td>
                        ...
                        {/* Close date */}
                        <td className="cell-mono" style={{ color:'var(--text2)' }}>{row.close_date ?? '—'}</td>
      ...
      {showAdd && (
        <AddRecordModal
          table="non_conformance_reports" title="إضافة تقرير NCR جديد"
          ...
          <div className="modal" style={{ width:440 }}>
            ...
            <div style={{
              ...
              <div style={{ color:'var(--text2)', marginBottom:12 }}>
                سيُؤرشف التقرير الحالي بحالة C وتُضاف مراجعة جديدة في الجدول مباشرة:
              </div>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ width:460 }}>
            ...
            <div style={{ fontSize:12, color:'var(--text3)', marginBottom:20 }}>
              💡 لحذف هذا التقرير احذف المراجعات الأحدث أولاً بالترتيب من الأعلى.
            </div>
